...
namespace OfficeOpenXml
{
  ...
  public enum eWorkSheetHidden
    ...
    public class ExcelWorksheet : XmlHelper, IEqualityComparer<ExcelWorksheet>, IDisposable
    {
        ...
        {
            var ix = Workbook.Styles.NamedStyles.FindIndexByID("標準");
            if (ix >= 0)
            {
        ...
        private void LoadCells(XmlReader xr)
        {
            ...
            while (!xr.EOF)
            {
                ...
                else if (xr.LocalName == "c")
                {
                    ...
                    {
                        style = 0;
                        SetStyleInner (address._fromRow, address._fromCol, -1);
                    }
                    xr.Read();
                }
...
#region Worksheet Save
        ...
        {
            var val = _values.GetValue (row, col);
            if (val._styleId < 0)
            {
                return true;
            }
            return (val._styleId != 0);
        }
        /// <summary>
        /// Existance check of sheet value
        /// </summary>

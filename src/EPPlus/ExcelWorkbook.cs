...
namespace OfficeOpenXml
{
	...
	public sealed class ExcelWorkbook : XmlHelper, IDisposable
	{
				...
				foreach (XmlElement elem in nl)
				{
					try
					{
						...
						else
						{
							...
							if (localSheetID > -1)
							{
								...
								{
									namedRange = Worksheets[localSheetID + _package._worksheetAdd].Names.Add(
										elem.GetAttribute("name"),
										new ExcelRangeBase(this, Worksheets[localSheetID + _package._worksheetAdd], fullAddress, false));
								}
								else
								{
									namedRange = Worksheets[localSheetID + _package._worksheetAdd].Names.Add(
										elem.GetAttribute("name"),
										new ExcelRangeBase(this, Worksheets[addr._ws], fullAddress, false));
								}
							}
							else
							{
					...
					catch (Exception ex)
					{
						System.Diagnostics.Debug.WriteLine(
							$"GetDefinedNames skipped: name='{elem.GetAttribute("name")}', value='{elem.InnerText}', ex={ex.Message}");
					}
				}
			}
		}
		...
		public decimal MaxFontWidth 
		{
			get
			{
                var ix = Styles.NamedStyles.FindIndexByID("標準");
                if (ix >= 0)
                {
		...
		private void UpdateDefinedNamesXml()
		{
			try
			{
				...
				else
				{
					...
					{
            if (name.IsName
                && !string.IsNullOrEmpty (name.NameFormula)
                && (name.NameFormula.StartsWith ("[file:")
                    || name.NameFormula.StartsWith ("[https:")))
              continue;
